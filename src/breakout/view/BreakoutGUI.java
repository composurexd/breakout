...
public class BreakoutGUI extends Application implements IEventHandler {
    ...
    private void keyPressed(KeyEvent event) {
        ...
        // HAR skrivit till kod att förflytta i X-LED
        switch (kc) {
            case LEFT:
                // Om vi kan förflytta den i x-Led utan att skära igenom väggen
                if(paddle.canMoveToPositionLeft()){
                    paddle.setX(paddle.getX() - Paddle.MOVEMENT_IN_X_AXIS);
                }
                break;
            case RIGHT:
                if(paddle.canMoveToPositionRight()){
                    paddle.setX(paddle.getX() + Paddle.MOVEMENT_IN_X_AXIS);
                }
                break;
            default:  // Nothing
    ...
    private void newGame() {
        ...
        List<Brick> bricks = getBricks(400,400);
        bindBricks(bricks);


        breakout = new Breakout(new Ball(), bricks, new Paddle(), getWalls());
   //     breakout = new Breakout(new Ball(), bricks, new Paddle(), new Wall(Wall.Dir.VERTICAL,0));
    ...
    private List<Wall> getWalls() {
        Wall left = new Wall(Wall.Dir.VERTICAL, 0);             // TODO
        Wall top = new Wall(Wall.Dir.HORIZONTAL, 0);               //TODO
        Wall right = new Wall(Wall.Dir.VERTICAL,400);             //TODO
